...
FTPID=${BANKID}
FTPIP=${BANKIP}
LogFile="${LogDir}/emr2bank-837.log"
FTPCmd="${TempDir}/emr2bank-837.ftp"
LockFile="${TempDir}/emr2bank-837.lock"
OffsiteDir="/mnt/remote"
OffsiteTestFile="${OffsiteDir}/offline.txt"
OffsiteShare="//srv-winserver/share"
SourceDir="/mnt/emr/bank/outbound/837"
TargetDir="/bak/bank/outbound/archive/emr/837"
TargetFTP="/home/user/837toBank"
AppTitle="emr2bank-837"
...
if [ ! -f ${OffsiteTestFile} ]; then
  ...
  if [ ! -f ${OffsiteTestFile} ]; then
    ## Unmount failed.
    echo "`date +%Y-%m-%d_%H:%M:%S` ** ERROR: Cannot unmount remote share folder. (Exit Code 3)" >> ${LogFile}
    f_cleanup
    exit 3
  fi
fi
